 ...
 * @type boolean
 * @desc 利用脚下显示HP量规的功能
 * 初期値: ON ( false = OFF 关闭显示 / true = ON 显示 )
 * @default false
 *
 * @param floorDamage
 * @text 受到的伤害
 ...
 * 使用方法:
 *   详细内容请参考分发网站
 *   这个插件在RPG制作MV version1.5.0中进行动作确认
 *   这个插件是在MIT许可证下发布的
 *   改造、再分发等，可以自由使用！
 ---------------------------------------------
 敌人快速设置：
 ---------------------------------------------
 敌人备注：<dead:A><repop:120>  
 敌人注释：<enemy:1>
 敌人漂浮注释：<gravity:0>
 ------------------------------------------
 重力平台快速：<lift><gravity:0><weight:99>
 * -----------------------------------------
 *   备注栏标签（角色、装备、状态）:
 * -----------------------------------------
 *   <move_speed:0.05>        # 歩行速度
 *   <jump_speed:0.14>        # 弹跳力
 *   <swim_speed:0.02>        # 游泳的速度
 *   <ladder_speed:0.04>      # 梯子移动速度
 *   <accele:0.003>           # 步行加速度
 *   <ladder_accele:0.003>    # 梯子移动加速度
 *   <jump_input:0>           # 追加输入时间
 *   <swim_jump:0.1>          # 水中弹跳力
 *   <mulch_jump:1>           # 连续跳跃次数
 *   <weight:2>               # 重量
 ...
 *   <dash_count:15>          # 冲刺时间
 *   <dash_delay:30>          # 冲刺后的硬度时间
 *   <dash_mp_cost:0>         # 冲刺所需的MP
 *   <fall_guard:50>          # 抗坠落损伤
 *   <guard_speed:15>         # 向防御状态过渡的速度
 *   <invincible_time:30>     # 伤害后的无敌时间
 *   <shot_way:1>             # 同时发射的子弹数量
 *   <shot_space:0.2>         # 子弹与子弹之间的间距（弧度）
 *   <shot_speed:0.07>        # 子弹飞行速度
 *   <shot_count:30>          # 子弹显示寿命
 *   <shot_type:1>            # 子弹类型
 *   <shot_index:0>           # 子弹图片的索引
 *   <shot_skill:1>           # 子弹技能编号
 *   <shot_delay:10>          # 子弹再次发射间隔时间 数值越小越快
 *   <shot_se_name:Attack2>   # 子弹发射声音效果的文件名
 *   <shot_se_volume:90>      # 子弹发射的声音效果的音量
 *   <shot_se_pitch:150>      # 子弹发射的声音效果的音调
 ------------------------------------
武器（枪）
<shot_way:1>
<shot_index:2>
...
<shot_count:10>
<shot_delay:50>
<shot_se_name:CrossBow>
 * 备注栏标签(事件):
 * 
 *   <w:0.375>                # 命中率决定（从中心到左或右边缘的大小）
 *   <h:0.75>                 # 命中率确定（从脚到头的大小）
 *   <enemy:1>                # 敌人编号
 *   <dead:A>                 # 敌人阵亡自我切换
 *   <repop:300>              # 重新出现的时间（帧）
 *   <lift>                   # 提升属性
 ...
 * 备注栏标签(技能):
 * 
 *   <bullet_anime:67>        # 子弹落地时播放的动画
 *   <map_through>            # 子弹无视地形，可以穿透
 *   <map_reflect>            # 当子弹击中地形时，它们不会消失，而是会跳弹
 * 
 *   <time_bomb:6 0 0.2 45 1 0 1>
 *     当子弹因时间耗尽而被删除时，发射新的子弹
 *     参数是插件命令 "nallShot "的n至技能Id设置。
 *
 * 
 * 插件命令:
 * 
 *   actGainHp -1 -5          # 对玩家造成5点伤害
 *   actGainHp 1 -100         # 1号事件造成100点伤害
 *   actHp 1 2                # 将事件1号的HP代入游戏变量2号。
 *   actForceX -1 0.1         # 玩家的X速度强制变更为0.1
 *   actForceY 1 -0.15        # 强制改变Y轴速度为-0.15，用于1号事件
 *   actForceStop -1          # 强制改变球员的速度为0
 *   actChangeActor 2         # 将操作角色改为2号演员
 *   actHideHpGauge           # 隐蔽式脚踏式水压测量仪
 *   actShowHpGauge           # 显示脚下的HP表
 * 
 *   actPopup -1 テキスト #ff0000
 *     向玩家弹出红色文本
-------------------------------------------------------------
 *   直线攻击敌人代码：nwayShot 3 1 0 3.14159 0.1 30 1 1 1
     敌人近战配置：nwayAim 3 1 0.2 0 0.1 10 2 7 9
-------------------------------------------------------------
反弹技能备注：
<map_reflect>
<time_bomb:6 0 0.1 20 1 3 12>
<gravity:0.004>
-------------------------------------
结束弹<map_through>
 *   nwayShot eventId n space angle speed count type index skillId
 *     eventId: 发射子弹的事件的编号（玩家为-1）
 *     n:       同时发射的子弹数量00000
 *     space:   子弹与子弹之间的间距（弧度）
 *     angle:   发射方向（弧度）0：右  1.57089：正下  2：左下  3：左下 3.14159：正左
 *     speed:   子弹飞行速度 数值越大越快 默认0.1
 *     count:   子弹寿命
 *     type:    子弹类型
 *     index:   子弹图片的索引
 *     skillId: 子弹技能（用于计算伤害，可以简略）
 * 
 *   nwayAim eventId n space angle speed count type index skillId
 *     nway_shot同样，在angle中玩家所在的方向(镭射
 *     自动加起来；如果ANGLE是0，它就会瞄准自己的机器
 *
 *   nallShot eventId n angle speed count type index skillId
 *     向所有方向发射子弹，子弹之间的间隔会自动设定
 *
 *   nallAim eventId n space angle speed count type index skillId
 *     nall_shot 这是一个自我定位的版本
 */
